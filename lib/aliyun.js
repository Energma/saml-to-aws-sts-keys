...
function saml2aliyun(details, OS) {
    let SamlPayload = "";
    let TargetRole = "";
    let SamlProvider = "";
    let output_text = "empty";
    console.log("detected aliyun saml login, url:", details.url, "os:", OS);
...
`
    if (OS == "non-windows") {
        output_text = script_text_linux
    } else {
        output_text = script_text_powershell
    }
    outputDocAsDownload(output_text);
    return;
}
